...
import json
from structures.blocks.data import Dataset
from structures.blocks.reference import Reference
from structures.blocks.preprocessing import Features
from structures.high_level import FullPaper
...
def find_all_features(d):
    ...
    for feats in _find_all(d, "features"):
        out.update(Features(feats).get_base_feature_names())
    return out
    ...
    if 'flows' in d and d['flows'] is not None:
        for flow in d['flows']:
            ...
            if feats is not None:
                out.update(Features({'features': feats}).get_base_feature_names())
    return out
...
def find_all_operations(d):
    out = set()
    for feats in _find_all(d, "features"):
        out.update(Features(feats).get_operation_names())
    return out
